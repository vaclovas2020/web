...
func (vm *VM) InitVM(ctx context.Context, args []string, sourceDir string) {
	...
	for _, file := range files {
		if !file.IsDir() {
			wg.Add(1)
			go vm.parseFileWorker(wg, fmt.Sprintf("%v/%v", sourceDir, file.Name()), ctx, output, errCh)
		}
	}
...
func (vm *VM) parseFileWorker(wg *sync.WaitGroup, fileName string, ctx context.Context, output chan<- string, errCh chan<- string) {
	defer wg.Done()
